...
CONTEXTS: dict[str, list[Step]] = {
    ...
    "asm": _asm(1),
    "asm_lines": _asm(1)
    + [("draw_3d_line", {"x1": 0.0, "y1": 0.0, "z1": 0.0, "x2": 0.3, "y2": 0.0, "z2": 0.0})],
    "asm2": _asm(2),
...
CASES: list[Case] = [
    ...
    Case(
        "draw_3d_line",
        {"x1": 0.0, "y1": 0.0, "z1": 0.0, "x2": 0.1, "y2": 0.0, "z2": 0.05},
        "part",
    ),
    Case(
        "sketch_constraint",
...
def write_report(rows: list[Row], started: dt.datetime, version: str, done: bool) -> None:
    ...
    lines = [
        ...
        "",
        f"{len(rows)} cases across {tools_hit} of {_tool_total()} tools. "
